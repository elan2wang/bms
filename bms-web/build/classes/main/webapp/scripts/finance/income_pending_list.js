var income_pending_list_api = "/bmp/1/incomes/pending_list";
//var income_pending_list_api = "/bmp/views/pending_list.json";

var get_handle_departments_api = "/bmp/1/incomes/get_handle_departments";

$(function(){
	getHandleDepartments();
	init(income_pending_list_api);
	$("#btn_search_income").click(function(){
		var condition = get_search_condition({"conditions":["income_type","bring_type","invoice_type","invoice_state","department","order_type"]});
		init(income_pending_list_api + "?" +condition);
	});
});

function init(url) {
	$("#income_pending_list").html('<tr><td colspan="12" style="text-align:center;"><img alt="正在加载" src="../images/loading.gif" />正在加载列表，请稍等...</td></tr>');
	$.ajax({
		url: url,
		type: "get",
		data: {},
		...
		success: function(result) {
			if(typeof(result.error_code) != "undefined"){
				$("#income_pending_list").html('<tr><td colspan="12" style="text-align:center;"><span style="color:red;">'+result.error_msg+'</span></td></tr>');
			}
				...
				if(items.length > 0 && parseInt(currentItemCount) > 0){
					...
					$("#income_pending_list").html(table_str);
				} else {
					$("#pagination").html("");
					$("#income_pending_list").html('<tr><td colspan="12" style="text-align:center;">暂无数据</td></tr>');
				}
			}
		}
	});
