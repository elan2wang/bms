			...
			if(typeof(result.error_code) != "undefined"){
				...
				if (currentItemCount > 0) {
					...
					for (var i=0; i<items.length; i++) {
						var id = items[i].id;
						titles += '<tr><td><input id="id'+id+'" name="id" type="checkbox" onclick="unselectall()" value="'+id+'"/></td>';
						titles += '<td>'+(i+1)+'</td>';
						...
						titles += '<td>'+state+'</td>';
						titles += '<td><a class="edit" href="javascript:void(0);" onclick="edit_invoice_title('+id+');return false;" >编辑</a>';
						titles += '<a class="edit" href="javascript:void(0);" onclick="delete_invoice_title('+id+');return false;" >删除</a>';
						titles += '<a class="edit" href="javascript:void(0);" onclick="switch_enable('+id+','+enable+');return false;" >'+op+'</a>';
						titles += '</td></tr>';
					}
